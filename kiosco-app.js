    ...
    /** Vista empresa: lista de fundadores = administradores (role super; dupla sin staff aparte). */
    async function loadFounderAdministradoresCard() {
      var card = document.getElementById('founderAdminsCard');
      var listEl = document.getElementById('founderAdminsList');
      var badge = document.getElementById('founderAdminsCountBadge');
      var hint = document.getElementById('founderAdminsHint');
      if (!card || !listEl) return;
      if (!currentUser || currentUser.role !== 'super' || !isEmpresaLensSuper()) {
        card.classList.add('hidden');
        return;
      }
      function mergeSuperRows(queryRows) {
        var q = queryRows && queryRows.slice ? queryRows.slice() : [];
        var cache = window._ferriolAllProfilesCache;
        var map = new Map();
        if (cache && Array.isArray(cache)) {
          cache.filter(function (x) { return x.role === 'super'; }).forEach(function (r) { map.set(r.id, r); });
        }
        q.forEach(function (r) { map.set(r.id, r); });
        return Array.from(map.values()).sort(function (a, b) { return String(a.email || '').localeCompare(String(b.email || '')); });
      }
      card.classList.remove('hidden');
      listEl.innerHTML = '<li class="text-white/35 text-xs py-2 px-1">Cargando fundadores (administradores)…</li>';
      if (badge) badge.textContent = '…';
      if (hint) hint.classList.add('hidden');
      if (!supabaseClient) {
        listEl.innerHTML = '<li class="text-amber-200/80 text-xs px-1">Sin conexión a Supabase.</li>';
        return;
      }
      try {
        if (!window._ferriolAllProfilesCache || window._ferriolAllProfilesCache.length === 0) {
          var prefetch = await supabaseClient
            .from('profiles')
            .select('id, email, role, active, kiosco_name, trial_ends_at, sponsor_id, referral_code');
          window._ferriolAllProfilesCache = (!prefetch.error && prefetch.data) ? prefetch.data : [];
        }
        var res = await supabaseClient
          .from('profiles')
          .select('id, email, active, trial_ends_at, kiosco_name')
          .eq('role', 'super')
          .order('email', { ascending: true });
        if (res.error) console.warn('loadFounderAdministradoresCard select:', res.error);
        var rows = mergeSuperRows((!res.error && res.data) ? res.data : []);
        listEl.innerHTML = '';
        if (badge) badge.textContent = rows.length + ' / 2';
        if (hint) hint.classList.toggle('hidden', rows.length === 2);
        if (rows.length === 0) {
          var empty = document.createElement('li');
          empty.className = 'rounded-xl border border-amber-500/25 bg-black/25 px-3 py-3 text-xs text-amber-100/85';
          empty.textContent = 'Aún no aparecen los dos fundadores como administradores (role super). Cada uno: usuario en Authentication + fila en profiles (véase ferriol-foundadores-dos-administradores.sql). Sin eso esta lista está vacía.';
        ...
        rows.forEach(function (p) {
          ...
          li.className = 'rounded-xl border border-white/10 bg-black/25 px-3 py-2.5 flex flex-col gap-1.5';
          var top = document.createElement('div');
          top.className = 'flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1';
          var em = document.createElement('span');
          em.className = 'text-sm font-semibold text-white break-all';
          var emailStr = (p.email || '(sin email)').trim();
          em.textContent = emailStr + (p.id === currentUser.id ? ' · vos' : '');
          top.appendChild(em);
          var st = document.createElement('span');
          st.className = 'text-[10px] font-semibold uppercase tracking-wide shrink-0 ' + (p.active ? 'text-emerald-300' : 'text-red-300/95');
          st.textContent = p.active ? 'Activa' : 'Bloqueada';
          top.appendChild(st);
          li.appendChild(top);
          var tl = trialLabel(p.trial_ends_at);
          var nm = document.createElement('p');
          nm.className = 'text-[11px] text-white/50';
          nm.textContent = 'Cuenta regresiva · vigencia: ' + (!p.trial_ends_at ? 'sin fecha (configurá trial_ends_at)' : tl.text) +
            (p.trial_ends_at ? (tl.days <= 0 ? ' · renovar ya' : ' restantes') : '');
          li.appendChild(nm);
        ...
        errLi.className = 'text-amber-200/85 text-xs px-1 py-2';
        errLi.textContent = 'No se pudo leer la lista de fundadores-administradores. Revisá conexión o RLS en profiles; necesitás dos cuentas super (vos y el otro fundador) en profiles.';
        listEl.appendChild(errLi);
        if (badge) badge.textContent = '?';
        console.warn('loadFounderAdministradoresCard:', e);
      }
      try { if (typeof lucide !== 'undefined' && lucide && lucide.createIcons) lucide.createIcons(); } catch (_) {}
    }
    ...
    async function loadSuperIngresosSection() {
      ...
      await loadFounderAdministradoresCard();
